  ...
  return (
    <aside className={`sidebar p-3 ${open ? 'open' : ''}`}>
      ...
      <nav className="nav flex-column gap-1">
        <NavLink className={({isActive})=>`nav-link px-3 py-2 ${isActive?'active':''}`} to="/dashboard" onClick={onClose}><FaChartLine className="me-2"/>Dashboard</NavLink>
        ...
        <NavLink className={({isActive})=>`nav-link px-3 py-2 ${isActive?'active':''}`} to="/prediction" onClick={onClose}><FaChartLine className="me-2"/>Prediction</NavLink>
